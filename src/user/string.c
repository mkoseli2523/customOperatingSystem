...
/**
 * this function finds the last occurance of a char in a string
 * 
 * @param s         the list of characters we are searching
 * @param c         the character to compare to
 * 
 * @return          returns a pointer to the last occurance of the char c
 */
char *strchr (const char * s, int c) {
    while (*s != '\0') {
        if (*s == (char)c) {
            return (char *)s;   // return a pointer 
        }
		s++;
    }

    // if we can't find the character return NULL
    return NULL;    
}
...
/**
 * this function tokenizes the given string one token at a time
 * 
 * set the input to NULL after providing an initial string to get the next token
 * 
 * @param str       string to tokenize
 * @param delim     tokenize each occurance of
 * 
 * @return          returns 
 */
char *strtok (char *str, const char *delim) {
	...
	// if a new string is provided update the state
	if (str != NULL) {
		last = str;
	}
	...
	// if the end of string is reached return NULL
	if (*start == '\0') {
		last = NULL;
		return NULL;
	}
	...
	// if a delimeter is found NULL terminate the string
	if (*end != '\0') {
		*end = '\0';
		last = end + 1;
	} else {
		// if the end of the string is reached set last to NULL
		last = NULL;
	}
 ...
 *            (no negative or non-digit handling in this version)
 *
 * @param str   A null-terminated string containing only digits [0-9].
 * @return      The integer value of the string, or 0 if empty string.
 */
int parse_int(const char *str) {
    ...
    while (*str != '\0') {
        ...
        // shift current result and add this digit
